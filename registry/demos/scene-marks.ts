...
import { type Story, storyTimeline } from "../items/story.ts";

export type SceneMark = { name: string; from: number; background?: string };

/** Where each scene starts, in frames: a scene begins as the previous scene's fade starts. */
export function sceneMarks(story: Story): SceneMark[] {
  const { frames, overlaps } = storyTimeline(story);
  let from = 0;
